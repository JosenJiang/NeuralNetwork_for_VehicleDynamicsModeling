...
def data_process():
    # 1. 生成训练数据 tstep = 8 ms
    path = "/home/plusai/Documents/workspace/NeuralNetwork_for_VehicleDynamicsModeling/inputs/trainingdata/data_to_train_0.csv"
    df = pd.read_csv(path)
    length = len(df)
    input_column = [
        "vx_mps",
    ...
    output_data = df.loc[:, output_column]  # 输出特征
...
def plot(args, predicted_output, test_output_data, name="test_model.png"):
    ...
    if args.savepic:
        pic_path = os.path.join(args.output, "model", name)
...
def model_evaluation(args):
    ...
    test_length = 3000
    plot_predicted_output = predicted_output_tensor[:test_length]  # why?
...
def model_training(args):
    ...
    for epoch in tqdm.trange(num_epochs):
        ...
        if loss_mean < 1e-4:
            print('Epoch [{}/{}], Loss: {:.5f}'.format(epoch+1, num_epochs, loss_mean))
            print("The loss value is reached")
            break
        if (epoch + 1) % 5 == 0:
            print(
                "Epoch [{}/{}], Loss: {:.4f}".format(
                    epoch + 1, num_epochs, loss_mean
                )
            )
    ...
    with torch.no_grad():
        predicted_output = model(train_input_data)
        predicted_output = predicted_output.to("cpu").numpy()
